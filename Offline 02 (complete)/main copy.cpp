...
void stage4() {
  ...
  for (int i = 0; i < triangles.size(); i++) {
    ...
    for (double row_no = start_row; row_no <= end_row; row_no++) {
      ...
      double x[3], z[3];
      for (int j = 0; j < 3; j++) {
        if (t.p[j]->y != t.p[(j+1)%3]->y) {
          double temp = (y - t.p[j]->y) / (t.p[(j+1)%3]->y - t.p[j]->y);
          x[j] = t.p[j]->x + temp * (t.p[(j+1)%3]->x - t.p[j]->x);
          z[j] = t.p[j]->z + temp * (t.p[(j+1)%3]->z - t.p[j]->z);

          if ((x[j]>t.p[j]->y && x[j]<t.p[(j+1)%3]->y || x[j]<t.p[j]->y && x[j]>t.p[(j+1)%3]->y) == false) {
            x[j] = __DBL_MAX__;
          }
        } else {
          x[j] = __DBL_MAX__;
      ...
      // sort x and z
      double left_scanline, right_scanline, z_left, z_right;
      for (int j=0; j<3; j++) {
        if (x[j]==__DBL_MAX__ && x[(j+1)%3]==__DBL_MAX__ && x[(j+2)%3]!=__DBL_MAX__) {
          if (x[j] < x[(j+1)%3]) {
            left_scanline = x[j];
            right_scanline = x[(j+1)%3];
            z_left = z[j];
            z_right = z[(j+1)%3];
          } else {
            left_scanline = x[(j+1)%3];
            right_scanline = x[j];
            z_left = z[(j+1)%3];
            z_right = z[j];
          }
      ...
      // nothing to do if all non-intersections
      if (x[0]==__DBL_MAX__ && x[1]==__DBL_MAX__ && x[2]==__DBL_MAX__) {
        continue;
      }
      ...
      for (double col_no = start_col; col_no <= end_col; col_no++) {
        ...
        if (left_scanline == right_scanline) {
          z = z_left;
        } else {
          z = z_left + (z_right - z_left) * (x - left_scanline) / (right_scanline - left_scanline);
        }
